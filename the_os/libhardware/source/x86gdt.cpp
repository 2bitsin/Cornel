...
auto x86arch::gdt_table_resize(std::pmr::memory_resource& allocator, std::uint16_t new_size, std::uint64_t defval, std::uint32_t flags) -> std::span<std::uint64_t>
{
  using namespace memory;
  x86arch::interrupt_guard cli;
  auto curr_gdt = x86arch::sgdt(); 
  curr_gdt = reallocate_buffer(allocator, curr_gdt, new_size, defval, flags);
