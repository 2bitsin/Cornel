...
auto vfsio::archive::open(error& error_v, type_id_enum type_v, std::string_view name_v) -> bool
{
	...
	if (!m_stack.empty())
	{
		...
		base_offset_v = offset_v + header_size(header_v);
	}
...
auto vfsio::archive::size(error& error_v) -> std::uintmax_t
{
	error_v = error::none;
	if (m_stack.empty ()) {
		error_v = error::invalid_operation;
	...
	auto const& [offset_v, header_v] = m_stack.top();
	auto const data_offset_v = offset_v + header_size(header_v);
	auto const archive_size_v = m_block->size(error_v);
	if (error_v != error::none)
		return 0;
	return archive_size_v - data_offset_v;
}
...
auto vfsio::archive::close(error& error_v, type_id_enum type_v) -> bool
{
	error_v = error::none;
	if (m_stack.empty()) {
		error_v = error::invalid_operation;
