...
namespace bits
{
  ...
  {
    using type = decltype(detail::pick_type<(0+...+_Length), 0>());
    return detail::Packer<type, 0, _Length...>::pack(args...);
  }
  ...
  {
    //using type = decltype(detail::pick_type<(0+...+_Length), 0>());
    detail::Unpacker<Q, _Length...>::unpack(q, std::forward<_Args>(args)...);
  }

  template <std::size_t... _Length, typename Q>
  constexpr auto unpack_as_tuple(Q q)
  {
    std::tuple result {detail::pick_type<_Length, 0>()...};
    detail::Unpacker<Q, _Length...>::unpack_tuple(q, result, std::make_index_sequence<sizeof...(_Length)>{});
    return result;
  }
  ...
  {
    using value_type = decltype(detail::mask<0, _Length>());
    constexpr const auto m = detail::mask<0, _Length>();
  ...
  {   
    constexpr auto _Mask = detail::mask<_Offset, _Size>();
